...
def save_data(data):
    try:
        with ensure_conn().cursor() as cursor:
            cursor.execute("insert into tb_data(job_position, job_company, job_salary, job_address, job_info) values(%s, %s, %s, %s, %s)", data[0])
    except pymysql.MySQLError as err:
        print(err)
...
def get_data(url):
    ...
    if not resp.url.startswith("https://www.jobui.com"):
        data.append(("no", "no", "no", "no", "no"))
        return data
    soup = bs4.BeautifulSoup(resp.text, "lxml")
    # job_position
    keyword = soup.select_one("#wrapcontainer > div.astruct.cfix > div.aleft.cfix > div.job-info.box > div.jk-box.jk-matter.j-job-detail > h1")
    # job_info, job_advantage
    print(keyword.text)
    # job_company
    company = soup.select_one("#companyH1 > a")
    print(company.text)
    # job_salary
    salary = soup.select_one("#wrapcontainer > div.astruct.cfix > div.aleft.cfix > div.job-info.box > div.jk-box.jk-matter.j-job-detail > ul > li:nth-child(3) > span.fs16.fwb.f60")
    print(salary.text)
    # job_address
    address = soup.select_one("#wrapcontainer > div.astruct.cfix > div.aleft.cfix > div.job-info.box > div.jk-box.jk-matter.j-job-detail > ul > li:nth-child(1)")
    print(address.text)
    # job_info
    infos = soup.select_one("#wrapcontainer > div.astruct.cfix > div.aleft.cfix > div.job-info.box > div.jk-box.jk-matter.j-job-detail > div.cfix > div")
    description = infos.text
    print(description)
    # advantage = soup.select_one("#job_detail > dd.job-advantage")
    data.append((clear_data(keyword.text), clear_data(company.text), salary.text, clear_data(address.text[5:]), clear_data(description)))
    return data
...
def main():
    ...
    try:
        for page in range(0, 200000, 1000):
            urls = get_db_urls("tb_zhiyouji", page, 1000)
            for url in urls:
                operation_data(url)
            save_page(page)
    except:
